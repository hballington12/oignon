...
- [ ] `[H+C]` GitHub Pages transition. NOT a blind redirect: hballington12.github.io/oignon and oignon.dev are separate origins, so a redirect strands existing users' localStorage (bookmarks, recents, cached graphs) — violates the additive-cache constraint. Decide: keep GH Pages serving + "we moved" banner, or build a localStorage export/import bridge first, then redirect. See open decision.
