...
- [ ] `[H]` Build the real landing page in Claude design, drop into `landing/` (see `landing/README.md`). Copy built on the two wedges: open-source/no-signup/private, and "paste your .bbl, see what your bibliography is missing before your reviewer does." Then `[H+C]` test on preview + merge.
